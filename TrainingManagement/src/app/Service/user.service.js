"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var http_1 = require("@angular/http");
require("rxjs/add/operator/map");
var UserService = (function () {
    function UserService(http) {
        this.http = http;
        this.baseUrl = "http://localhost:3040/api/";
        this.headers = new http_1.Headers({ 'Content-Type': 'application/json' });
    }
    UserService.prototype.addNewUser = function (user) {
        return this.http.post(this.baseUrl + '', JSON.stringify(user), this.headers)
            .map(function (response) { return response.json().data; });
    };
    UserService.prototype.assignRole = function (userName, role) {
        return null;
    };
    UserService.prototype.getUsers = function () {
        return this.http.get(this.baseUrl + '')
            .map(function (response) { return response.json().data; });
    };
    UserService.prototype.getUsersByRole = function (role) {
        return this.http.get(this.baseUrl + '')
            .map(function (response) { return response.json().data; });
    };
    UserService.prototype.getUser = function (userName) {
        return this.http.get(this.baseUrl + '' + userName)
            .map(function (response) { return response.json().data; });
    };
    UserService.prototype.authenticate = function (userName, password) {
        console.log('service: authenticating');
        return this.http.post(this.baseUrl + 'authenticate', { username: userName, password: password }, this.headers).map(function (response) { return response.json(); });
    };
    return UserService;
}());
UserService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [http_1.Http])
], UserService);
exports.UserService = UserService;
//# sourceMappingURL=user.service.js.map